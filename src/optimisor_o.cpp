			...
			for(int y = (int)minmin.y; y<=(int)maxmax.y; y++){
				for(int x = (int)minmin.x; x<=(int)maxmax.x; x++){
					...
					double inv_pos[2] = { (1-s) * ((1-t)*in_quad[0][0].x+t*in_quad[1][0].x) + s * ((1-t)*in_quad[0][1].x+t*in_quad[1][1].x), 
						(1-s) * ((1-t)*in_quad[0][0].y+t*in_quad[1][0].y) + s * ((1-t)*in_quad[0][1].y+t*in_quad[1][1].y) };
					int fip[2] = { (int)floor(inv_pos[0]), (int)floor(inv_pos[1]) }; // floored inverse position
					if(fip[0]<0 || fip[1]<0 || fip[0]>=in_s.width-2 || fip[1]>=in_s.height-2) continue; // pixel out of image
					double s_2 = inv_pos[0] - fip[0];
					double t_2 = inv_pos[1] - fip[1];
					Vec3b nb[2][2] = { { in_img.at<Vec3b>(fip[1], fip[0]), in_img.at<Vec3b>(fip[1], fip[0]+1)},
					{ in_img.at<Vec3b>(fip[1]+1, fip[0]), in_img.at<Vec3b>(fip[1]+1, fip[0]+1)} }; // 4 neighbor pixels
	...
	for(size_t p = 0; p < corr.size(); ++p) {
		...
		for(int i = 0; i < 2; ++i){
			...
			*  alpha3-- up  , alpha4--down	 */
			...
			feat[i].alpha[0] = alpha1*alpha3;	feat[i].alpha[1] = alpha2*alpha3;
			feat[i].alpha[2] = alpha1*alpha4;	feat[i].alpha[3] = alpha2*alpha4;
...
void Optimisor::depthConstraint(vector< vector<double> >& matrix_val, vector<double>& b, int& row_count, vector<Vec3d>& depth_points, int num){
	...
	for(size_t p = 0; p < depth_points.size(); ++p) {
		...
		*  alpha3-- up  , alpha4--down	 */
		...
		/*if(b1<0||b1>1||b2<0||b2>1||b3<0||b3>1||(b1+b2+b3)!=1)
		{
			cout<<"pos:"<<pos.x<<","<<pos.y<<endl;
			cout<<"pos:"<<vertpos[idx].x<<","<<vertpos[idx].y<<endl;
			cout<<"pos:"<<vertpos[idx+1].x<<","<<vertpos[idx+1].y<<endl;
			cout<<"pos:"<<vertpos[idx+nv_col+1].x<<","<<vertpos[idx+nv_col+1].y<<endl;
			cout<<"pos:"<<vertpos[idx+nv_col+2].x<<","<<vertpos[idx+nv_col+2].y<<endl;
		}*/
...
#endif
		feat.alpha[0] = alpha1*alpha3;	feat.alpha[1] = alpha2*alpha3;
		feat.alpha[2] = alpha1*alpha4;	feat.alpha[3] = alpha2*alpha4;
...
void Optimisor::distConstraint(vector< vector<double> >& matrix_val, vector<double>& b, int img, int& row_count){
	...
	for(int r = 0; r < num_row; ++r) {
		for(int c = 0; c < num_col; ++c) {
			...
			distort.idx_ver_r.resize(4);	distort.idx_ver_c.resize(4);
			distort.idx_ver_r[0] = r;		distort.idx_ver_r[1] = r;
			distort.idx_ver_r[2] = r+1;		distort.idx_ver_r[3] = r+1;
			distort.idx_ver_c[0] = c;		distort.idx_ver_c[1] = c+1;
			distort.idx_ver_c[2] = c;		distort.idx_ver_c[3] = c+1;
#if LOOP_GRID
			if(c == num_col-1)
			{
